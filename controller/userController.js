...
exports.placeOrder = async (req, res) => {
  ...
  for (const product of products) {
    ...
    if (!productDetails) {
      return res
        .status(400)
        .send(`Product with ID ${product.productId} not found`);
    }
    const quantity = Number(product.quantity);
    if (isNaN(quantity) || quantity <= 0) {
      return res
        .status(400)
        .send(`Invalid quantity for product ${productDetails.name}`);
    }
    if (productDetails.stock < quantity) {
      return res
        .status(400)
        .send(`Insufficient stock for product ${productDetails.name}`);
    }
  ...
  return res
    .status(201)
    .send({ msg: "Order placed successfully", orderId: savedOrder._id });
};
...
exports.addToSale = async (req, res) => {
  ...
  if (discountPercentage <= 0 || discountPercentage > 100) {
    return res
      .status(400)
      .send("Discount percentage must be between 1 and 100");
  }
  ...
  return res
    .status(201)
    .send({ msg: "Sale added successfully", discount: savedDiscount });
};
...
exports.detailsOfProduct = async (req, res) => {
  ...
  const product = await Product.findById(productId).select(
    "-createdAt -updatedAt -deletedAt -isDeletedFromCart -hashVariants -lowStockThreshold -isActive -ratings -materials -options -variants"
  );
  if (!product) {
    return res.status(400).send("No product found with this ID");
  }
